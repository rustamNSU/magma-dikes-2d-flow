...
frame_dir = os.path.join(repository_dir, "images", f"temperature_frames_{'_'.join(map(str, simIDs))}")
...
for it in range(timesteps):
    ...
    fig = plt.figure(figsize=(2.5*len(simIDs), 5.5), layout="constrained")
    ...
    for ax, cax, dike in zip(axs, caxs, dikes):
        data = dike.data[it]
        ax.set_title(r"\bf Temperature (C$^\circ$)")
        ...
        pcmT = ax.pcolormesh(y2db, x2db, T, shading='flat', cmap='jet', vmin=Tlim[0], vmax=Tlim[1])
        cbT = fig.colorbar(pcmT, cax=cax)
