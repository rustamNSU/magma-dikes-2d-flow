...
for i in range(len(simIDs)):
    ...
    lTm, = ax3.semilogy(X, Tm, linewidth=2, label=legends[i] + ", mid.", linestyle='-', color=colors[i])
    lTb, = ax3.semilogy(X, Tb, linewidth=2, label=legends[i] + ", bound.", linestyle='--', color=colors[i])
    lws.append(lw)
    lps.append(lp)
    lTms.append(lTm)
    lTbs.append(lTb)
